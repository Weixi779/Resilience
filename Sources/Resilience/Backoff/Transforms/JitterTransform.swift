...
/// - Uses the provided random number generator for testability; defaults to the system generator via `Backoff.duration`.
///
/// Example:
/// ```swift
...
struct JitterTransform: BackoffTransform {
    ...
    func apply<Generator: RandomNumberGenerator>(
        ...
        let factor = Double.random(in: low...high, using: &randomNumberGenerator)
